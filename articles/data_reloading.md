...
In the [`run_app()`](../reference/run_app.md) call we specify the
duration after which the datasets should be reloaded via the
...
`"touching"` an `"restart.txt"` file and therefore the `"shiny-server"`
will then cover the reloading of the application and therefore a
reloading of the datasets.
