...
1. **unfiltered_dataset**: a reactive list containing the tables inside the selected dataset before filtering them.
  
1. **filtered_dataset**: a reactive list containing the tables inside the selected dataset after filtering them.
...
The call to afmm is included in a reactive, because `afmm[["unfiltered_dataset]]` and `afmm[["filtered_dataset]]` are reactives, therefore, they can only be accessed inside a reactive environment. Also note the parenthesis resolving the reactive before doing the subsetting. This does not differ from usual reactive programming.
