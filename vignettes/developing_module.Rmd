...
Then, we pass the whole `filtered_dataset_list` list to the `dataset` parameter in the server function because we are no longer specifying just one data set - in this way, we can allow the user to select the data set inside the application at run-time.
