...
This filter has two different interfaces a *Basic* one and a *Advanced* one. Both have the same mission, but the 
...
<div class="alert alert-info" role="alert">
Two other opt-in ways of selecting the subpopulation exist alongside the Filter tab: **Subgroup**
(`enable_subgroup = TRUE`, see `vignette("subgroup")`) and **Chat** (`enable_querychat = TRUE`, a
natural-language, `querychat`-powered filter, see `vignette("querychat_filter")`). All three combine
via intersection — enabling more than one only ever narrows the resulting population further.
</div>
...
<div class="alert alert-info">
  **Note:**  
  Datasets can be assigned a human readable label by adding an attribute `label` to the datasets inside a dataset_list.
</div>
...
Now let's switch to the **Advanced** filter. To switch between filter types, use the dropdown menu in the top section (marked as 1).
...
In the animation above, a basic filter is created for the `adae` dataset to include only rows where the *Lowest Level Term* is *ABDOMINAL CRAMPS*.
...
For example, you can create filters on the `adae` dataset such as:
  ...
  ![](images/variable_operations_and.png){width=100%}

  - *Lowest Level Term* is *ABDOMINAL CRAMPS* **or** the *Severity* is *SEVERE*
  ![](images/variable_operations_or.png){width=100%}

  - *Lowest Level Term* is *ABDOMINAL CRAMPS* **or** the *Severity* is **not** *SEVERE*
  ![](images/variable_operations_or_not.png){width=100%}
...
For example:
  ...
  ![](images/set_operations_union_complement.png){width=100%}
...
![](images/searching.png){width=100%}
