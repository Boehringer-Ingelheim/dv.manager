local({
  ...
  test_that(
    "char_vars_to_factor_vars_dataset transform character variable into factors variables" |>
      vdoc[["add_spec"]](c(specs$PREPROCESSING$PREPROCESS_CHAR_TO_FACTOR)),
    {
  ...
  test_that(
    "char_vars_to_factor_vars respect lables" |>
      vdoc[["add_spec"]](c(specs$PREPROCESSING$PREPROCESS_CHAR_TO_FACTOR)),
    {
  ...
  test_that(
    "decorate_char_vars_to_factor_vars_dataset_list decorates a functions that returns a list of data.frames and transforms character variables into factor variables" |>
      vdoc[["add_spec"]](c(specs$PREPROCESSING$PREPROCESS_CHAR_TO_FACTOR)),
    {
      # nolintr
      f <- function() dl
  ...
  test_that(
    "char_vars_to_factor_vars_dataset_lists applies decorators and transformations to dataset_lists" |>
      vdoc[["add_spec"]](c(specs$PREPROCESSING$PREPROCESS_CHAR_TO_FACTOR)),
    {
  ...
  test_that(
    "character variables are transformed into factors during run_app calls" |>
      vdoc[["add_spec"]](c(specs$PREPROCESSING$PREPROCESS_CHAR_TO_FACTOR)),
    {
      r <- suppressMessages(
        suppressWarnings(
      ...
      expect_identical(
        r[["config"]][["afmm_static"]][["data"]][["dl1"]][["ds1"]],
        expected_dataset_lists[["dl1"]][["ds1"]]
      )
      expect_identical(
        r[["config"]][["afmm_static"]][["data"]][["dl1"]][["ds2"]],
        expected_dataset_lists[["dl1"]][["ds2"]]
      )
      expect_identical(
        r[["config"]][["afmm_static"]][["data"]][["dl2"]]()[["ds1"]],
        expected_dataset_lists[["dl2"]]()[["ds1"]]
      )
      expect_identical(
        r[["config"]][["afmm_static"]][["data"]][["dl2"]]()[["ds2"]],
        expected_dataset_lists[["dl2"]]()[["ds2"]]
      )
    }
  )
})
...
local({
  ...
  test_that(
    "ungroup2df_datasets_dataset_list ungroups and transforms into data.frames datasets in a dataset_list" |>
  ...
  test_that(
    "decorate_ungroup2df_datasets_dataset_list decorates a functions that returns a list of data.frames and ungroups and transforms into data.frames all datasets in the list" |>
      vdoc[["add_spec"]](c(specs$PREPROCESSING$PREPROCESS_UNGROUP, specs$PREPROCESSING$PREPROCESS_TIBBLE_TO_DF)),
    {
      # nolintr
      f <- function() dl
  ...
  test_that(
    "ungroup2df_datasets_dataset_lists applies decorators and transformations to dataset_lists" |>
      vdoc[["add_spec"]](c(specs$PREPROCESSING$PREPROCESS_UNGROUP, specs$PREPROCESSING$PREPROCESS_TIBBLE_TO_DF)),
    {
