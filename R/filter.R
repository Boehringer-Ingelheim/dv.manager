...
to_filter_validate <- function(x) {jsonvalidate::json_validate(x, system.file("to_filter_schema.json", package = "dv.manager", mustWork = TRUE), engine = "ajv", strict = FALSE, verbose = TRUE)}
from_filter_validate <- function(x) {jsonvalidate::json_validate(x, system.file("from_filter_schema.json", package = "dv.manager", mustWork = TRUE), engine = "ajv", strict = FALSE, verbose = TRUE)}
...
new_filter_ui <- function(id, data) {
  ...
  js_template <- "
    $('#%s').data('filter', blockly_filter.init('%s'));
    $(document).ready(function() {      
      const send_code = function() {
        const filter = $('#%s').data('filter');
        const code = blockly_filter.get_code(filter.workspace, filter.generator);          
        Shiny.setInputValue('%s', code, {priority: 'event'});
      };
    document.getElementById('%s').addEventListener('click', send_code);
    });
  "
  ...
  shiny::div(
    ...
    shiny::br(),
    shiny::tags[["button"]](id = ns("gen_code"), "Apply filter", class = "btn btn-primary btn-lg"),
    shiny::div(
      id = "code"
    )
  )
...
new_filter_server <- function(id, data, strict = FALSE) {
...
mock_new_filter <- function(data = list(
  ...
  server <- function(input, output, session) {
    x <- new_filter_server("filter", shiny::reactive(data), strict = TRUE)
    selected_data <- "D1"
